...
let homeWithFile: string; // fake home with .kongcode/cuda-visible-devices
...
beforeAll(async () => {
  ...
  await mkdir(join(homeWithFile, ".kongcode"), { recursive: true });
...
describe("gpu-pin (opt-in, no-op by default)", () => {
  ...
  it("applies KONGCODE_CUDA_VISIBLE_DEVICES and defaults CUDA_DEVICE_ORDER=PCI_BUS_ID", () => {
    const env: NodeJS.ProcessEnv = { KONGCODE_CUDA_VISIBLE_DEVICES: UUID };
    const r = applyGpuPin(env, homeEmpty);
    expect(r).toMatchObject({ applied: true, value: UUID, source: "env:KONGCODE_CUDA_VISIBLE_DEVICES" });
  ...
  it("leaves an operator-set CUDA_VISIBLE_DEVICES untouched (no override)", () => {
    const env: NodeJS.ProcessEnv = { CUDA_VISIBLE_DEVICES: "0", KONGCODE_CUDA_VISIBLE_DEVICES: UUID };
    const r = applyGpuPin(env, homeWithFile);
    expect(r.applied).toBe(false);
    expect(env.CUDA_VISIBLE_DEVICES).toBe("0");
  });

  it("applies the ~/.kongcode/cuda-visible-devices file", () => {
    const env: NodeJS.ProcessEnv = {};
    const r = applyGpuPin(env, homeWithFile);
  ...
  it("env knob beats the file", () => {
    const env: NodeJS.ProcessEnv = { KONGCODE_CUDA_VISIBLE_DEVICES: "GPU-other" };
    const r = applyGpuPin(env, homeWithFile);
    expect(r.source).toBe("env:KONGCODE_CUDA_VISIBLE_DEVICES");
    expect(env.CUDA_VISIBLE_DEVICES).toBe("GPU-other");
  });

  it("keeps a pre-set CUDA_DEVICE_ORDER", () => {
    const env: NodeJS.ProcessEnv = { KONGCODE_CUDA_VISIBLE_DEVICES: UUID, CUDA_DEVICE_ORDER: "FASTEST_FIRST" };
