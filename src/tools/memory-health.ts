...
/** T5 (2026-06-10): every count is `number | null` — null means "the count
 *  QUERY failed", which is materially different from 0 ("table is empty").
 *  The old defaultVal=0 lie let a failing store report a green, zero-gap
 *  substrate. Consumers (UI, scripts) treat null as unknown. */
interface HealthMetrics {
  concept_count: number | null;
  concept_embedded: number | null;
  memory_count: number | null;
  memory_embedded: number | null;
  turn_count: number | null;
  turn_embedded: number | null;
  artifact_count: number | null;
  artifact_embedded: number | null;
  retrieval_outcome_count: number | null;
  pending_work_count: number | null;
  embedding_gap_pct: number | null;
}
...
export async function handleMemoryHealth(
  ...
  try {
    const storePath =
      process.env.LAQRUMCODE_STORE_PATH ??
  ...
  let embedding_gap_pct: number | null = null;
  if (gapInputs.every((v): v is number => v !== null)) {
  ...
  if (distMtimeAtStartup > 0 && DAEMON_ENTRYPOINT_PATH) {
    try {
      ...
      if (currentMtime > distMtimeAtStartup + 1000 /* 1s slack */) {
        ...
        diagnostics.push({
          ...
          message:
            `daemon dist drift detected: ${DAEMON_ENTRYPOINT_PATH} mtime advanced ` +
    ...
    } catch (e) { swallow("memoryHealth:distMtime", e); }
  }
