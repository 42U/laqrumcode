...
const DEFAULT_IMPORTANCE: Record<FindingType, number> = {
...
export async function handleRecordFinding(
  state: GlobalPluginState,
  session: SessionState,
  args: Record<string, unknown>,
): Promise<{ content: Array<{ type: "text"; text: string }> }> {
  const findingType = String(args.finding_type ?? "").trim() as FindingType;
