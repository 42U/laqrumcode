...
let _sharedLlama: Llama | null = null;
...
export async function getSharedLlama(profile?: ResourceProfile): Promise<Llama> {
  if (_sharedLlama) return _sharedLlama;
  if (_sharedLlamaInit) return _sharedLlamaInit;
...
export async function disposeSharedLlama(): Promise<void> {
  if (_sharedLlamaInit) {
    try { await _sharedLlamaInit; } catch { /* ignore */ }
  }
  if (_sharedLlama) {
    try { await _sharedLlama.dispose(); } catch { /* ignore */ }
    _sharedLlama = null;
  }
  _sharedLlamaInit = null;
}
