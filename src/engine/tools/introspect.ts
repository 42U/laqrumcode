...
const VERIFY_SENSITIVE_FIELDS = new Set<string>([
  "cleanup_claim_token",
  "auth_token",
  "credentials",
  "password",
...
const USER_CONTENT_FIELDS = new Set<string>([
...
function redactUserContent(value: unknown): string {
...
function maskAndTruncate(value: string, len = 300): string {
  let masked = value;
  for (const pat of SECRET_PATTERNS) {
    masked = masked.replace(pat, "[redacted-secret-pattern]");
  }
...
function deepRedact(value: unknown, depth = 0, parentKey?: string): unknown {
  if (depth > 4) {
...
async function verifyAction(store: any, recordId?: string) {
  ...
  const lines = Object.entries(cleaned)
    .map(([k, v]) => {
      ...
      if (USER_CONTENT_FIELDS.has(k) && typeof v === "string") return `  ${k}: ${v}`;
      if (typeof v === "string") {
        let masked = v;
        for (const pat of SECRET_PATTERNS) masked = masked.replace(pat, "[redacted-secret-pattern]");
...
export function isConnectedDbExternal(connectedUrl: string): boolean {
  if (process.env.SURREAL_URL) return true;
  let port: number | null = null;
  try { port = Number(new URL(connectedUrl).port) || null; } catch { /* unparseable */ }
  if (port === null) return false; // can't tell → assume managed (we only walk our own dataDir)
  return !new Set<number>([pickPort(), LEGACY_MANAGED_SURREAL_PORT]).has(port);
}
