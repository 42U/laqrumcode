...
async function evaluateRetrievalInner(
  ...
  const ceByIndex: Array<number | null> = new Array(items.length).fill(null);
  const ceEligible: number[] = items
    .map((_, idx) => idx)
    .sort((a, b) => (items[b].finalScore ?? 0) - (items[a].finalScore ?? 0));
  const ceTargets: number[] = [];
  let ceTokenBudget = EVAL_CE_TOKEN_BUDGET;
  for (const idx of ceEligible) {
    if (ceTargets.length >= EVAL_CE_TOP_K) break;
    ...
    if (ceTargets.length > 0 && approxTokens > ceTokenBudget) break;
  ...
  try {
    ...
    const ids = rows.map((r) => r.memory_id as string);
    const existing = await store.queryFirst<{ memory_id: string }>(
      `SELECT memory_id FROM retrieval_outcome
         WHERE session_id = $sid AND turn_id = $tid AND memory_id IN $ids`,
      { sid: sessionId, tid: responseTurnId, ids },
    );
    const existingIds = new Set(existing.map((e) => String(e.memory_id)));
    const toInsert = rows.filter((r) => !existingIds.has(r.memory_id as string));
    if (toInsert.length > 0) {
      ...
      await store.queryExec(
        `INSERT INTO retrieval_outcome $rows ON DUPLICATE KEY UPDATE memory_id = memory_id`,
        { rows: toInsert },
      );
    }
  } catch (e) {
...
export async function getLastTurnGroundingTrace(
  ...
  try {
    ...
    const latest = await store.queryFirst<{ turn_id: string }>(
...
export function computeSignals(
  ...
  if (!cited && utilization < 0.4) {
    ...
    if (pathHit || identHit) utilization = Math.max(utilization, 0.4);
  }
...
function extractPaths(text: string): string[] {
  const paths: string[] = [];
  for (const m of text.matchAll(PATH_RE)) paths.push(m[1]);
  return paths;
}

function extractBacktickIdents(text: string): string[] {
  const idents: string[] = [];
  for (const m of text.matchAll(/`([A-Za-z][\w.]{2,40})`/g)) {
    if (!STOP_WORDS.has(m[1].toLowerCase())) idents.push(m[1]);
  }
  return idents;
}
