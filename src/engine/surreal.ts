...
function consolidateKeepOuter(
  outerImp: number, innerImp: number,
  outerSecondary: number, innerSecondary: number,
  outerId: string, innerId: string,
): boolean {
  if (outerImp !== innerImp) return outerImp > innerImp;
  if (outerSecondary !== innerSecondary) return outerSecondary > innerSecondary;
...
function sortByTimestampAsc<T extends { timestamp?: unknown }>(rows: T[]): T[] {
...
export function isRetryableSurrealError(e: unknown): boolean {
...
export function patchOrderByFields(sql: string): string {
  const s = sql.trim();
  ...
  const selIdx = (selectMatch as unknown as { indices: Array<[number, number]> }).indices[1];
  ...
  if (!orderMatch) return sql; // the only ORDER BY lives inside a subquery — outer query needs nothing
  const ordIdx = (orderMatch as unknown as { indices: Array<[number, number]> }).indices[1];
  ...
  for (const part of splitTopLevel(selectClause)) {
    ...
    if (aliasMatch) selectedFields.add(aliasMatch[1].toLowerCase());
    const expr = (aliasMatch ? part.slice(0, aliasMatch.index) : part).trim();
    const last = expr.split(".").pop()!.trim().toLowerCase();
    if (/^[a-z_][a-z0-9_]*$/i.test(last)) selectedFields.add(last);
  }
...
export class SurrealStore {
  ...
  constructor(config: SurrealConfig, opts?: { skipSupervisorRegister?: boolean }) {
    this.config = config;
    this.db = new Surreal();
    ...
    if (!opts?.skipSupervisorRegister) registerSurrealSupervisorStore(this);
  }
  ...
  private async connectWithTimeout(): Promise<void> {
    await raceWithDeadline(
      ...
      SurrealStore.CONNECT_TIMEOUT_MS,
      "SurrealDB connect",
    );
  }
  ...
  private async recordSchemaWedgeRecovery(e: unknown): Promise<void> {
    const msg = String((e as { message?: string })?.message ?? e).slice(0, 300);
    const recoveryCmd = "node scripts/predeploy-dedup.mjs --apply";
    log.error(
      `[surreal] SCHEMA-UPGRADE-WEDGE: schema apply REJECTED by a UNIQUE index on ` +
    ...
    try {
      await this.queryExec(`CREATE maintenance_runs CONTENT $data`, {
        data: {
          ...
          duration_ms: 0,
          error:
            `Schema apply rejected by UNIQUE index on duplicate rows (upgrade across <0.7.70 with legacy data). ` +
  ...
  private async ensureConnected(): Promise<void> {
    ...
    if (this.db.isConnected && !this.zombieSuspect) return;
        ...
        try {
          ...
          if (!this.schemaApplied) {
            try {
              ...
              log.error(
                `[surreal] reconnect schema re-apply failed — store stays degraded until next heal: ${(e as Error).message}`,
              );
            }
          }
          ...
          if (this.schemaApplied) this.zombieSuspect = false;
          return;
        } catch (e) {
          if (attempt < MAX_ATTEMPTS) {
            await new Promise((r) => setTimeout(r, BACKOFF_MS[attempt - 1]));
      ...
      if (isTransactionConflict(e)) {
        ...
        for (const ms of BACKOFF_MS) {
          ...
          try {
            ...
            lastErr = e2;
            if (!isTransactionConflict(e2)) throw e2;
          }
        }
        throw lastErr;
      }
  ...
  private async deadlineQuery(
    fullSql: string,
    bindings?: Record<string, unknown>,
    ms = QUERY_DEADLINE_MS,
    opts: { flagZombie?: boolean } = {},
  ): Promise<unknown> {
    ...
    try {
      ...
      if (flagZombie && e instanceof Error && e.message.includes("deadline exceeded")) {
        ...
        log.error(
          `[surreal] query deadline exceeded after ${ms}ms — connection flagged zombie; ` +
  ...
  async queryFirst<T>(sql: string, bindings?: Record<string, unknown>): Promise<T[]> {
    ...
    return this.withRetry(async () => {
      ...
      const rows = Array.isArray(result) ? (result as T[][])[(result as T[][]).length - 1] : result;
  ...
  async queryBatch<T = any>(statements: string[], bindings?: Record<string, unknown>): Promise<T[][]> {
    ...
    return this.withRetry(async () => {
      ...
      const raw = await this.deadlineQuery(fullSql, bindings) as unknown[];
  ...
  async getSessionTurns(
    ...
    // absent.
    const rows = await this.queryFirst<{ role: string; text: string; timestamp?: string }>(
      `SELECT role, text, timestamp FROM turn WHERE session_id = $sid AND pruned_at IS NONE ORDER BY timestamp DESC LIMIT $lim`,
  ...
  async getSessionTurnsRich(
    ...
    const rows = sortByTimestampAsc(await this.queryFirst<{ id: string; role: string; text: string; tool_name?: string; tool_result?: string; file_paths?: string[]; timestamp?: string }>(
      `SELECT id, role, text, tool_name, tool_result, file_paths, timestamp FROM turn WHERE session_id = $sid AND pruned_at IS NONE ORDER BY timestamp DESC LIMIT $lim`,
      { sid: sessionId, lim: limit },
    ));
  ...
  async relate(fromId: string, edge: string, toId: string): Promise<boolean> {
  ...
  async createTask(description: string, projectId?: string): Promise<string> {
    ...
    const data: Record<string, unknown> = { description, status: "in_progress" };
    if (projectId) data.project_id = projectId;
    const rows = await this.queryFirst<{ id: string }>(
      `CREATE task CONTENT $data RETURN id`,
      { data },
    );
    return String(rows[0]?.id ?? "");
  }
  ...
  async createSession(agentId = "default", kcSessionId?: string, projectId?: string): Promise<string> {
    ...
    const data: Record<string, unknown> = { agent_id: agentId };
    if (kcSessionId) data.kc_session_id = kcSessionId;
    if (projectId) data.project_id = projectId;
    const rows = await this.queryFirst<{ id: string }>(
      `CREATE session CONTENT $data RETURN id`,
      { data },
    );
    return String(rows[0]?.id ?? "");
  }
  ...
  async fetchAccessDeltas(ids: Array<string | unknown>): Promise<Map<string, number>> {
    ...
    try {
      ...
      const stats = await this.queryFirst<{ target: string; hits: number }>(
        `SELECT <string>target AS target, hits FROM ${statTargets}`,
      );
      if (stats.length === 0) return out;
      const hitIds = stats.map(s => String(s.target));
      const watermarks = await this.queryFirst<{ id: string; synced_hits?: number }>(
        `SELECT <string>id AS id, synced_hits FROM ${hitIds.join(", ")}`,
      );
      const synced = new Map(watermarks.map(w => [String(w.id), w.synced_hits ?? 0]));
      for (const s of stats) {
        const delta = (s.hits ?? 0) - (synced.get(String(s.target)) ?? 0);
        if (delta > 0) out.set(String(s.target), delta);
      }
    } catch (e) {
  ...
  async upsertConcept(
    ...
    if (existingId) {
      ...
      if (embedding?.length) {
        await this.queryExec(
          `UPDATE ${id} SET embedding = $emb WHERE embedding IS NONE OR array::len(embedding) = 0`,
          { emb: embedding },
        );
      }
      if (projectId) {
        await this.queryExec(
          `UPDATE ${id} SET project_id = $pid WHERE project_id IS NONE`,
          { pid: projectId },
        );
      }
    ...
    try {
      ...
      if (isUniqueViolation(createErr)) {
        ...
        if (existing[0]?.id) return { id: String(existing[0].id), existed: true };
      ...
      if (path && isUniqueViolation(createErr)) {
        ...
        try {
          ...
          if (isUniqueViolation(retryErr)) {
            ...
            if (existingAgain[0]?.id) return { id: String(existingAgain[0].id), existed: true };
  ...
  async createMemory(
    ...
    if (text && text.length > 0) {
      ...
      if (exact.length > 0) {
        ...
        await this.queryExec(
          `UPDATE ${existingId} SET importance = $imp WHERE importance IS NONE OR importance < $imp`,
          { imp: importance },
        );
        return existingId;
      }
    ...
    if (embeddingTarget && embeddingTarget !== text) record.embedding_target = embeddingTarget;
    const rows = await this.queryFirst<{ id: string }>(
  ...
  async getUtilityFromCache(ids: string[]): Promise<Map<string, number>> {
    ...
    try {
      ...
      const rows = await this.queryFirst<{
        ...
        `SELECT memory_id, util_sum, retrieval_count, avg_utilization FROM memory_utility_cache WHERE memory_id IN $ids`,
        { ids: recIds },
      );
      for (const row of rows) {
        const mean = utilityMean(row);
        if (mean != null) result.set(String(row.memory_id), mean);
      }
    } catch (e) {
  ...
  async getUtilityCacheEntries(ids: string[]): Promise<Map<string, UtilityCacheEntry>> {
    ...
    try {
      ...
      const rows = await this.queryFirst<{
        ...
        `SELECT memory_id, util_sum, avg_utilization, retrieval_count FROM memory_utility_cache WHERE memory_id IN $ids`,
        { ids: recIds },
      );
      for (const row of rows) {
        const mean = utilityMean(row);
  ...
  private async recordMaintenanceRun(
    ...
    try {
      ...
      if (error) data.error = error.slice(0, 300);
  ...
  async runMemoryMaintenance(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < MAX_BATCHES; i++) {
        ...
        const res = await this.queryMulti<{ count: number; last: unknown }>(
          `LET $batch = (
           ...
           RETURN { count: array::len($batch), last: array::last($batch).id };`,
          { batch: BATCH, ...(cursor != null ? { cursor } : {}) },
        );
        const count = Number(res?.count ?? 0);
        touched += count;
        if (count < BATCH || res?.last == null) break;
        cursor = res.last;
      }
  ...
  async garbageCollectMemories(): Promise<number> {
    ...
    try {
      ...
      const pruned = await this.queryMulti<number>(
        `LET $stale = (
          SELECT id FROM memory
   ...
   */
  async purgeOldRetrievalOutcomes(): Promise<number> {
    const RETAIN = 30_000;
    const started = Date.now();
    try {
      const countRows = await this.queryFirst<{ count: number }>(
        `SELECT count() AS count FROM retrieval_outcome GROUP ALL`,
      );
      const count = countRows[0]?.count ?? 0;
      // Only act when meaningfully over target (avoid churn right at the bound).
      if (count <= RETAIN + 5_000) return 0;
      if (!(await this.shouldRunMaintenance("purgeOldRetrievalOutcomes", 60, 1, count))) return 0;
      ...
      const cutoffRows = await this.queryFirst<{ created_at: string }>(
        `SELECT created_at FROM retrieval_outcome ORDER BY created_at DESC LIMIT 1 START ${RETAIN}`,
      );
   ...
   */
  async purgeOldTurnScores(): Promise<number> {
    const RETAIN = 30_000;
    const started = Date.now();
    try {
      const countRows = await this.queryFirst<{ count: number }>(
        `SELECT count() AS count FROM turn_score GROUP ALL`,
      );
      const count = countRows[0]?.count ?? 0;
      // Only act when meaningfully over target (avoid churn right at the bound).
      if (count <= RETAIN + 5_000) return 0;
      if (!(await this.shouldRunMaintenance("purgeOldTurnScores", 60, 1, count))) return 0;
      // created_at of the RETAIN-th most-recent row → delete everything older
      // (uses ts_created_idx for the ORDER BY and the DELETE predicate).
      const cutoffRows = await this.queryFirst<{ created_at: string }>(
        `SELECT created_at FROM turn_score ORDER BY created_at DESC LIMIT 1 START ${RETAIN}`,
      );
      const cutoff = cutoffRows[0]?.created_at;
      if (!cutoff) return 0;
  ...
  async purgeOldMaintenanceRuns(): Promise<number> {
    const RETAIN = 10_000;
    const started = Date.now();
    try {
      const countRows = await this.queryFirst<{ count: number }>(
        `SELECT count() AS count FROM maintenance_runs GROUP ALL`,
      );
      const count = countRows[0]?.count ?? 0;
      if (count <= RETAIN + 5_000) return 0;
      if (!(await this.shouldRunMaintenance("purgeOldMaintenanceRuns", 60, 1, count))) return 0;
      const cutoffRows = await this.queryFirst<{ ran_at: string }>(
        `SELECT ran_at FROM maintenance_runs ORDER BY ran_at DESC LIMIT 1 START ${RETAIN}`,
      );
    ...
    try {
      ...
      for (const ref of embReflections) {
        ...
        for (const dupe of dupes) {
          ...
          const keepRef = consolidateKeepOuter(
            ref.importance, dupe.importance, 0, 0,
            String(ref.id), String(dupe.id),
          );
      ...
      for (const sk of embSkills) {
        ...
        for (const dupe of dupes) {
          ...
          const keepSk = consolidateKeepOuter(
            sk.success_count ?? 0, dupe.success_count ?? 0, 0, 0,
            String(sk.id), String(dupe.id),
          );
  ...
  async getReflectionSessionIds(sessionIds?: string[]): Promise<Set<string>> {
    if (sessionIds !== undefined) {
      const ids = [...new Set(sessionIds.filter(Boolean).map(String))];
      if (ids.length === 0) return new Set();
      try {
        const rows = await this.queryFirst<{ session_id: string }>(
          `SELECT session_id FROM reflection WHERE session_id IN $ids GROUP BY session_id`,
          { ids },
        );
    ...
    try {
      ...
      const rows = await this.queryFirst<{ session_id: string }>(
        `SELECT session_id FROM reflection GROUP BY session_id LIMIT 20000`,
      );
