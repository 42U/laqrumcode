...
export const REFLECTION_ANTI_THOROUGHNESS_RE =
  /should have (moved on faster|just acknowledged|rushed|skipped|moved faster)|overthinking it|too detailed|too thorough|rushed it less|acknowledge and move on|over-engineered|too verbose|more concise|shorter, punchier|burned (too many|most of the) tool|keep responses tighter/i;

export const REFLECTION_SAVE_SUMMARY_RE =
  /\ball saved\.|quoted ids from this turn|session totals:|\d+\s+concept gems across|edges_created\s*=\s*\d+/i;

export const REFLECTION_WORK_COMPLETION_RE =
  /fix verified end-to-end:|final loop summary|final state:|\d+\/\d+\s*tests|\d+\/\d+\s*postdeploy|calling it done/i;
 ...
 *   embedding.
 */
export function classifyReflection(text: string): "drop" | "downgrade" | "ok" {
  if (REFLECTION_ANTI_THOROUGHNESS_RE.test(text)) return "drop";
  if (REFLECTION_SAVE_SUMMARY_RE.test(text) || REFLECTION_WORK_COMPLETION_RE.test(text)) return "downgrade";
  return "ok";
}
