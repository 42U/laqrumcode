...
export function resolveGpuPin(
  ...
  if (fromEnv) return { applied: false, value: fromEnv, source: "env:KONGCODE_CUDA_VISIBLE_DEVICES" };
  try {
    const f = gpuPinFilePath(home);
    if (existsSync(f)) {
      const v = readFileSync(f, "utf8").trim();
      if (v) return { applied: false, value: v, source: "file" };
    }
...
export function applyGpuPin(
  ...
  if (!r.value || r.source === "env:CUDA_VISIBLE_DEVICES") return { applied: false };
