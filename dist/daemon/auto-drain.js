...
export function resolveClaudeBin(plat, probes) {
    if (plat === "win32") {
        ...
        if (where) {
            for (const line of where.split(/\r?\n/)) {
                ...
                if (cand && probes.fileExists(cand))
                    return cand;
            }
        }
...
export function drainSpawnNeedsShell(plat) {
    return plat === "win32";
}
...
const DRAIN_PROMPT = "Drain the LaqrumCode pending_work queue. Loop: call mcp__plugin_laqrumcode_laqrumcode__fetch_pending_work " +
