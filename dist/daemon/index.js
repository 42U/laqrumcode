    ...
    try {
        ...
        for (const dir of stale) {
            ...
            try {
                ...
                log.warn(`[daemon] failed to prune ${dir.name}: ${e.message}`);
            }
        }
        log.info(`[daemon] pruned ${stale.length} stale version(s) from plugin cache, kept ${DAEMON_VERSION}`);
    }
    catch (e) {
        log.warn(`[daemon] plugin cache prune failed: ${e.message}`);
    }
}
...
async function initializeStack() {
    ...
    if (!sessionReaperTimer) {
        sessionReaperTimer = setInterval(() => {
            try {
                ...
                const isLive = attached ? (sid) => attached.has(sid) : undefined;
    ...
    try {
        ...
        void ensureEdgeIndexes(store, config.paths.cacheDir).catch((e) => log.warn(`[daemon] ensureEdgeIndexes: ${e.message}`));
    ...
    if (config.reranker.enabled) {
        if (existsSync(config.reranker.modelPath)) {
            ...
            initReranker(config.reranker.modelPath).catch(e => log.warn(`[daemon] eager reranker init failed (will retry lazily): ${e.message}`));
...
function initDaemonHandshakeToken() {
    ...
    try {
        ...
        log.warn(`[daemon] couldn't persist handshake token at ${path}: ${e.message} — clients will be unable to authenticate over TCP`);
    }
    return token;
}
...
function removeOwnTokenFile() {
    ...
    try {
        ...
        log.warn(`[daemon] couldn't remove token file: ${e.message}`);
    }
}
async function main() {
    ...
    const gracefulCleanup = async (reason) => {
        ...
        try {
            ...
            log.warn(`[daemon] stopHttpApi: ${e.message}`);
        }
        ...
        if (globalState) {
            try {
                await globalState.awaitPendingTasks(3_000);
            }
            catch (e) {
                log.warn(`[daemon] awaitPendingTasks: ${e.message}`);
            }
        }
        if (globalState) {
            try {
                ...
                log.warn(`[daemon] shutdown: ${e.message}`);
            }
        }
        try {
