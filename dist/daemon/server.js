 ...
 */
import { createServer, connect as netConnect, Socket as NetSocket } from "node:net";
...
const PRE_AUTH_METHODS = new Set([
    "meta.handshake",
    "meta.health",
]);
...
export class DaemonServer {
    ...
    maxInFlight = Number(process.env.LAQRUMCODE_DAEMON_MAX_INFLIGHT) > 0
    ...
    maxConnections = Number(process.env.LAQRUMCODE_DAEMON_MAX_CONNECTIONS) > 0
    ...
    listenBacklog = Number(process.env.LAQRUMCODE_DAEMON_BACKLOG) > 0
    ...
    acceptPauseMs = Number(process.env.LAQRUMCODE_DAEMON_ACCEPT_PAUSE_MS) > 0
    ...
    async probeTcpOccupant(port) {
        ...
        return new Promise((resolve) => {
            ...
            let buffer = "";
            sock.on("connect", () => {
                const req = { jsonrpc: "2.0", id: 1, method: "meta.health", params: {} };
                if (this.opts.probeToken)
                    req.params.handshake = this.opts.probeToken;
                try {
            ...
            sock.on("data", (chunk) => {
                ...
                if (nl === -1) {
                    if (buffer.length > 64 * 1024)
                        done("foreign"); // never a laqrumcode health line
                    return;
                }
                try {
    ...
    async listen() {
        ...
        if (this.opts.tcpPort !== null && this.opts.tcpPort !== undefined) {
            ...
            try {
                ...
                const code = e.code;
                if (code !== "EADDRINUSE")
                    throw e; // unrelated bind failure — surface as-is
                ...
                if (occupant === "laqrumcode-daemon") {
                    ...
                    throw new TcpPortInUseError("laqrumcode-daemon", port, `TCP 127.0.0.1:${port} is already served by another laqrumcode daemon (singleton already running for this user). ` +
                ...
                throw new TcpPortInUseError("foreign", port, `TCP 127.0.0.1:${port} is in use by a FOREIGN (non-laqrumcode) process — it did not answer a meta.health probe. ` +
    ...
    markPendingSupersede() {
        if (this.pendingSupersede)
            return; // already flagged — don't re-arm the grace timer
        ...
        if (graceMs > 0 && this.opts.onSupersedeDeadline && !this.supersedeGraceTimer) {
            ...
            this.supersedeGraceTimer = setTimeout(() => {
                ...
                try {
                    this.opts.onSupersedeDeadline();
                }
                catch (e) {
                    this.opts.log.warn(`[daemon] onSupersedeDeadline callback threw: ${e.message}`);
                }
            }, graceMs);
    ...
    async dispatchLine(sock, line) {
        ...
        if (this.opts.requireHandshakeAuth &&
