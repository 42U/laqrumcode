...
export async function handleSessionStart(state, payload) {
    ...
    if (store.isAvailable()) {
        try {
            ...
            const cwd = payload.cwd ?? state.workspaceDir ?? process.cwd();
            ...
            try {
                const gradEvents = await store.queryFirst(`SELECT * FROM graduation_event WHERE acknowledged = false ORDER BY created_at DESC LIMIT 1`);
                if (gradEvents.length > 0) {
                    ...
                    if (soul) {
                        ...
                        try {
                            ...
                            await store.queryExec(`UPDATE ${evt.id} SET acknowledged = true, acknowledged_at = time::now(), acknowledged_session = $sid`, { sid: session.sessionId });
                        }
                        catch (e) {
    ...
    const parts = [wakeupText, pendingNote].filter((p) => !!p);
