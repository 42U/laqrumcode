...
export async function handleSubagentStop(state, payload) {
    ...
    try {
        ...
        if (!subagentId && hasToolUseCorrelation) {
            const rows = await store.queryFirst(`SELECT id FROM subagent WHERE correlation_key = $cid LIMIT 1`, { cid: rawToolUseId }).catch(() => []);
        ...
        if (subagentId) {
           ...
           result_summary = $result`, {
            ...
            const existing = await store.queryFirst(`SELECT id FROM subagent WHERE correlation_key = $cid LIMIT 1`, { cid: corrKey }).catch(() => []);
            if (existing[0]?.id) {
