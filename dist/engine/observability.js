...
export async function pruneRawMetrics(store, retentionDays = 30) {
    ...
    try {
        ...
        await store.queryMulti(`LET $stale = (SELECT id FROM orchestrator_metrics
 ...
 */
async function computeRawAnomalyFlags(store) {
...
function detectGraduationClose(ctx) {
