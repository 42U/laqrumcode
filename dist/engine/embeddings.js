...
export class EmbeddingService {
    ...
    l2Put(hash, vec) {
        ...
        this.store.queryExec(`UPSERT embedding_cache SET text_hash = $hash, embedding = $vec, model_version = $mv, pruned_at = NONE, prune_reason = NONE WHERE text_hash = $hash`, { hash, vec, mv: this.modelVersion }).catch(e => swallow("embeddings:l2Put", e));
    }
    ...
    recordDimGuardFailure(gotDim, expectedDim) {
    ...
    maxQueueDepth = Number(process.env.LAQRUMCODE_EMBED_QUEUE_MAX) > 0
    ...
    async computeAndSettle(item) {
        ...
        try {
            ...
            if (vec.length !== expectedDim) {
                ...
                throw new Error(`embed() produced a ${vec.length}-dim vector but the index/model dimension is ${expectedDim}` +
