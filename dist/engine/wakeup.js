...
// --- Depth signals ---
async function getDepthSignals(store) {
    ...
    try {
        const [sessRows, monoRows, memRows, spanRows] = await Promise.all([
            ...
            store.queryFirst(`SELECT count() AS count FROM memory GROUP ALL`).catch(() => []),
 ...
 */
export async function synthesizeWakeup(store, currentSessionId, workspaceDir) {
    ...
    if (!handoff && !handoffFile && monologues.length === 0 && identityChunks.length === 0 && previousTurns.length === 0)
        return null;
    const sections = [];
    // Depth awareness
    const depthLines = [];
    if (depth.sessions > 0)
        depthLines.push(`~${depth.sessions} sessions`);
    if (depth.memoryCount > 0)
        depthLines.push(`${depth.memoryCount} memories`);
    if (depth.monologueCount > 0)
        depthLines.push(`${depth.monologueCount} monologue traces`);
    if (depth.spanDays > 0)
        depthLines.push(`spanning ${depth.spanDays} day${depth.spanDays === 1 ? "" : "s"}`);
    ...
    if (handoff) {
        ...
        if (handoffFile.lastUserText)
            lines.push(`Last user message: ${handoffFile.lastUserText}`);
        if (handoffFile.lastAssistantText)
            lines.push(`Last assistant message: ${handoffFile.lastAssistantText}`);
    ...
    if (!handoff && monologues.length === 0 && previousTurns.length === 0)
        return null;
