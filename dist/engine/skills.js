...
export function smoothedSkillUtility(successCount, failureCount) {
...
export function shouldRecordSkillOutcome(engagement, toolSuccess) {
    if (toolSuccess === null)
        return null; // no tool outcome to judge
    if (engagement === null)
        return null; // reranker offline — can't attribute
    if (engagement < SKILL_ENGAGEMENT_MIN)
        return null; // response didn't engage this skill
 ...
 */
export async function findRelevantSkills(queryVec, limit = 3, store, opts) {
    ...
    try {
       ...
       ORDER BY score DESC LIMIT $lim`, { vec: queryVec, lim: poolSize });
        ...
        const pool = rows
            ...
            .map((r) => ({ row: r, emb: (Array.isArray(r.embedding) ? r.embedding : []), rel: Number(r.score) }));
