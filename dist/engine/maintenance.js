...
export function __resetBootstrapMaintenanceForTests() {
...
export async function runEmbeddingBackfills(state) {
    try {
...
async function sweepOrphanedEdges(state) {
    if (!state.store.isAvailable())
        return;
    try {
...
async function backfillTurnEmbeddings(state) {
    if (!state.store.isAvailable())
        return;
...
async function backfillMemoryEmbeddings(state) {
    if (!state.store.isAvailable())
        return;
    if (!state.embeddings.isAvailable())
        return;
...
async function purgeStaleEmbedCache(state) {
    ...
    try {
        ...
        for (let i = 0; i < MAX_BATCHES; i++) {
            const rows = await state.store.queryFirst(`SELECT id FROM embedding_cache
           WHERE created_at < time::now() - 30d AND pruned_at IS NONE
           LIMIT ${BATCH}`);
            const ids = idsOf(rows);
            if (ids.length === 0)
                break;
            await state.store.queryExec(`UPDATE embedding_cache SET pruned_at = time::now(), prune_reason = "stale_30d"
           WHERE id IN [${ids.join(", ")}]`);
            if (ids.length < BATCH)
                break; // last (partial) batch — backlog drained
        }
        ...
        let removed = 0;
        for (let i = 0; i < MAX_BATCHES; i++) {
            const rows = await state.store.queryFirst(`SELECT id FROM embedding_cache WHERE pruned_at IS NOT NONE LIMIT ${BATCH}`);
            const ids = idsOf(rows);
            if (ids.length === 0)
                break;
            ...
            removed += ids.length;
            if (ids.length < BATCH)
                break; // last (partial) batch — pruned backlog drained
        }
        if (removed > 0) {
