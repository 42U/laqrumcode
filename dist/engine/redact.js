...
export function parsePrivacyConfig(raw) {
    const cfg = { redactPatterns: [...SECRET_PATTERNS], ignorePaths: [], ignoreProjects: [] };
    ...
    if (Array.isArray(r.redact_patterns)) {
        for (const p of r.redact_patterns) {
            ...
            try {
                ...
                log.warn(`[privacy] ignoring invalid redact_pattern ${JSON.stringify(p)}: ${e.message}`);
            }
        }
    }
    if (Array.isArray(r.ignore_paths)) {
        cfg.ignorePaths = r.ignore_paths.filter((x) => typeof x === "string");
    }
    if (Array.isArray(r.ignore_projects)) {
        cfg.ignoreProjects = r.ignore_projects.filter((x) => typeof x === "string");
    }
    return cfg;
}
...
export function loadPrivacyConfig(force = false) {
    if (cached && !force)
        return cached;
    let cfg = { redactPatterns: [...SECRET_PATTERNS], ignorePaths: [], ignoreProjects: [] };
    ...
    if (existsSync(path)) {
        try {
            ...
            log.warn(`[privacy] failed to parse ${path} (${e.message}) — using built-in secret patterns only`);
        }
    }
    cached = cfg;
    return cfg;
...
export function redactSecrets(text, patterns = SECRET_PATTERNS) {
    if (!text)
        return text;
    let out = text;
...
export function containsSecret(text, patterns = SECRET_PATTERNS) {
...
export function isIgnoredProject(projectId, cfg) {
    if (!projectId)
        return false;
...
export function isIgnoredPath(path, cfg) {
    if (!path)
        return false;
...
export function _resetPrivacyCache() {
    cached = null;
}
