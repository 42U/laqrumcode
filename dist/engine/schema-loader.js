...
export function loadSchema() {
    ...
    const candidates = [];
    if (moduleDir) {
        candidates.push(join(moduleDir, "schema.surql"), join(moduleDir, "..", "schema.surql"), join(moduleDir, "..", "src", "engine", "schema.surql"), join(moduleDir, "..", "..", "src", "engine", "schema.surql"));
    }
    ...
    throw new Error(`laqrumcode: schema.surql not found in any candidate path. Last error: ${lastErr?.message ?? "unknown"}`);
}
