...
export function shouldRunCheck(turnCount, session) {
    ...
    if (session.currentConfig?.skipRetrieval)
        return false;
    return turnCount === 2 || (turnCount - 2) % 5 === 0;
}
export function getPendingDirectives(session) {
    return getState(session).pendingDirectives;
}
export function clearPendingDirectives(session) {
    getState(session).pendingDirectives = [];
}
export function getSessionContinuity(session) {
    return getState(session).sessionContinuity;
}
export function getSuppressedNodeIds(session) {
 ...
 */
// --- Response parsing ---
export function parseCheckResponse(text) {
    ...
    if (Array.isArray(raw.directives)) {
        for (const d of raw.directives.slice(0, 3)) {
            if (!d.type || !d.target || !d.instruction)
                continue;
            if (!DIRECTIVE_TYPES.has(d.type))
                continue;
            directives.push({
                type: d.type,
    ...
    if (Array.isArray(raw.grades)) {
        for (const g of raw.grades.slice(0, 30)) {
            if (!g.id || typeof g.relevant !== "boolean")
                continue;
            if (!VALID_RECORD_ID.test(g.id))
                continue;
            grades.push({
                id: String(g.id),
