import { chmodSync, createWriteStream, existsSync, mkdirSync, readdirSync, readFileSync, readlinkSync, statSync, writeFileSync, } from "node:fs";
...
export function registerSurrealSupervisorStore(store) {
...
function armSupervisorForFreshSpawn() {
...
export function __resetSupervisorForTest() {
    if (supervisorState.timer)
        clearTimeout(supervisorState.timer);
    supervisorState.shuttingDown = false;
    supervisorState.degraded = false;
...
function surfaceSupervisorDegraded(message) {
...
function registerUnexpectedExit(now = Date.now()) {
...
async function onManagedChildExit(reason) {
    ...
    if (crashLoop) {
        ...
        surfaceSupervisorDegraded(`managed SurrealDB child exited >= ${SUPERVISOR_MAX_RESTARTS} times within ` +
    ...
    log.warn(`[bootstrap] managed SurrealDB child ${reason} unexpectedly — respawning in ${delay}ms ` +
    ...
    try {
        ...
        log.error(`[bootstrap] managed SurrealDB respawn failed: ${e.message}`);
        supervisorState.respawning = false;
...
opts) {
    ...
    const connectTimer = setTimeout(() => {
        connectController.abort(new Error(`download connect timeout: no response headers within ${CONNECT_TIMEOUT_MS}ms for ${url}`));
    }, CONNECT_TIMEOUT_MS);
    let res;
    try {
    ...
    const armStallTimer = () => {
        ...
        stallTimer = setTimeout(() => {
            source.destroy(new Error(`download stalled: no data for ${INACTIVITY_MS}ms after ${bytes} bytes for ${url}`));
        }, INACTIVITY_MS);
    };
...
function surrealCredPath(cacheDir) {
...
export function getOrCreateManagedCred(cacheDir) {
    ...
    if (existsSync(path)) {
        try {
            ...
            log.warn(`[bootstrap] failed to read managed surreal cred file (${e.message}) — regenerating`);
        }
    }
    // Generate fresh.
    ...
    try {
        ...
        catch { /* best-effort; no-op on Windows */ }
    }
    ...
    try {
        ...
        log.warn(`[bootstrap] could not persist managed surreal cred to ${path}: ${e.message}`);
    }
    return cred;
}
...
function managedCredFileExists(cacheDir) {
 ...
 */
export function findListenerUidViaProc(port, procRoot = "/proc") {
    const wantInodes = new Set();
    for (const file of ["net/tcp", "net/tcp6"]) {
        ...
        for (const line of text.split(/\r?\n/).slice(1)) {
            ...
            const loopbackOrWildcard = h === "0100007F" ||
                h === "00000000" ||
 ...
 */
function findListenerUid(port) {
    if (typeof process.getuid !== "function")
        return null; // non-POSIX
    // 1. /proc fast path.
    ...
    try {
        const out = execFileSync("lsof", ["-t", `-iTCP@127.0.0.1:${port}`, "-sTCP:LISTEN"], { encoding: "utf-8", timeout: 2_000, stdio: ["ignore", "pipe", "ignore"] }).trim();
...
function readLiveOwnSurrealPid(cacheDir) {
    try {
...
export async function findExistingKongcodeSurreal(cacheDir, managedPort, user, pass, 
...
resolveOwnerUid = findListenerUid) {
    ...
    const candidates = Array.from(new Set([8000, 8042, managedPort, LEGACY_MANAGED_SURREAL_PORT]));
    ...
    for (const port of candidates) {
        ...
        if (ourUid !== null) {
            ...
            if (ownerUid !== null && ownerUid !== ourUid) {
                log.warn(`[bootstrap] port ${port} hosts a kongcode DB owned by uid ${ownerUid} ` +
            ...
            if (ownerUid === null && managedSurfacePorts.has(port)) {
                ...
                if (ourLivePid === null) {
                    log.warn(`[bootstrap] port ${port} hosts a kongcode DB but its owner UID could ` +
...
export async function spawnManagedSurreal(binPath, dataDir, port, user, pass, cacheDir) {
    ...
    child.on("exit", (code, signal) => {
        ...
        void onManagedChildExit(`exited (code=${code ?? "null"}, signal=${signal ?? "null"})`);
    });
    if (child.pid) {
...
export function pickPort() {
    ...
    if (!username)
        return LEGACY_MANAGED_SURREAL_PORT;
...
export async function bootstrap(input) {
    ...
    managedSurreal = await spawnManagedSurreal(surrealBinary.path, input.dataDir, port, managedCred.user, managedCred.pass, input.cacheDir);
