...
export class SurrealStore {
    ...
    async archiveOldTurns() {
        ...
        try {
            ...
            const staleRows = await this.queryFirst(`SELECT id FROM turn WHERE timestamp < time::now() - 7d AND pruned_at IS NONE AND <string>id NOT IN (SELECT VALUE memory_id FROM retrieval_outcome WHERE memory_table = 'turn') LIMIT 500 TIMEOUT 8s`);
            if (!staleRows.length)
                return 0;
