...
function maybeWipeIdleCache(session) {
    const now = Date.now();
...
async function hasInvestigatedBashCommand(state, session, command, matchedPattern) {
    // Internal cache key prefixed so it can't collide with file paths.
    const cacheKey = `__bash__:${matchedPattern}`;
    if (session._editGateChecked.has(cacheKey))
        return true;
 ...
 */
export async function checkFileEditGate(state, session, filePath) {
    if (!filePath)
        return null;
 ...
 */
export async function checkBashGate(state, session, command) {
    if (!command)
        return null;
    maybeWipeIdleCache(session);
