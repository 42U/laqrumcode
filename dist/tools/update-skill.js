...
export async function handleUpdateSkill(state, _session, args) {
    ...
    if (!name)
        return text("Error: `name` is required to identify the skill to update.");
    const { store, embeddings } = state;
    if (!store.isAvailable())
        return text("Error: database unavailable. Cannot update skill.");
    const existing = await store.queryFirst(`SELECT id, description, body FROM skill WHERE name = $name LIMIT 1`, { name });
    ...
    if (embeddings.isAvailable()) {
        try {
            ...
            swallow.warn("update-skill:embed", e);
        }
    }
    if (!reEmbedded)
