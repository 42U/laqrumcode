...
async function commitResults(item, results, state) {
    ...
    switch (item.work_type) {
        ...
        case "causal_graduate": {
            ...
            if (created > 0) {
                try {
                    const groups = await state.store.queryFirst(`SELECT chain_type, count() AS cnt FROM causal_chain
             WHERE success = true AND confidence >= 0.7 AND graduated_at IS NONE
             GROUP BY chain_type`);
                    const types = groups.filter(g => g.cnt >= 3).map(g => g.chain_type);
                    if (types.length > 0) {
                        await state.store.queryExec(`UPDATE causal_chain SET graduated_at = time::now()
               WHERE success = true AND confidence >= 0.7 AND graduated_at IS NONE
                 AND chain_type IN $types`, { types });
                    }
                }
                catch (e) {
                    swallow("pending-work:graduateWatermark", e);
                }
            }
